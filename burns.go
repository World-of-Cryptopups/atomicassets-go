...
func (a *AtomicAssets) GetBurns(options *GetBurnsQuery) (*BurnsProps, error) {
	...
	if options != nil {
		...
		u, _ = urljoin.UrlJoin(a.API_URL, BURNS_API, fmt.Sprintf("?%s", v.Encode()))
	} else {
		u, _ = urljoin.UrlJoin(a.API_URL, BURNS_API)
	}
...
func (a *AtomicAssets) GetBurnsAccount(account string, options *GetBurnsQuery) (*BurnsAccountProps, error) {
	...
	if options != nil {
		v, _ := query.Values(options)
		u, _ = urljoin.UrlJoin(a.API_URL, BURNS_API, account, fmt.Sprintf("?%s", v.Encode()))
	} else {
		u, _ = urljoin.UrlJoin(a.API_URL, BURNS_API, account)
	}
